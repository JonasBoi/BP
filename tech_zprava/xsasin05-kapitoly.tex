...
Brány jsou v~LSTM buňce celkem tři. Každá je složená z~vrstvy neuronové sítě se sigmoidální funkcí (\uv{S} v \ref{lstm_cell}) a~násobícího (\uv{$\times$} v \ref{lstm_cell}) operátoru. Brána je komponentou určující, kolik informace je možno propustit. Brány jsou popsány zleva doprava.\par
...
Před vstupem do prvního bloku jsou jednotlivé tokeny vstupního textu převedeny na vektory (word embeddings), ke kterým je přidáno poziční kódování\footnote{Poziční kódování identifikuje pořadí tokenu ve vstupním textu.}. Každý blok se pak skládá z~\uv{\emph{multi-head attention}} modulu a~plně propojené dopředné vrstvy. Obě části jsou následovány normalizační vrstvou.\par
...
\end{tabular}
\caption{Výskyt termů \uv{term} a~\uv{frequency} v~dokumentech $d_1$ a~$d_2$ a~ohodnocení jednotlivých dokumentů podle funkcí TF-IDF a~BM25 ($k_1 = 2$) \cite{stanford:retrieval}}
\label{tab:tf}
\end{table}
...
Dalším z možných problémů je také vázanost datasetu na časové období, kdy byl vytvořen/zveřejněn (2019). Příkladem může být otázka \uv{Kolik evropských států tvoří Evropskou unii?}, pro kterou po brexitu už odpověď \uv{28} není realitou.\par
Procházet ručně a vybrat pro odstranění takové otázky z celé datové sady je pro jednoho člověka náročné, a proto je tento aspekt zohledněn až při vyhodnocení výstupů systému, diskutován v podkapitole \ref{metriky}.
...
\subsection{Návrh části retriever}
...
\subsection{Návrh komponenty reader}
...
%V zásadě jsou si state of the art systémy v architektuře retriever\,--\,reader velmi podobné, s drobnými odchylkami (jako je reranker, extraktivní/generativní reader).
...
%=========================================================================
\section{Zpracování používaných dat}
...
%=========================================================================
\section{Implementace a~trénink komponenty Reader}
\label{reader_imp}
...
%=========================================================================
\section{Implementace komponenty Retriever}
\label{retriever_imp}
